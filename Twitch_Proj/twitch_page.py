...
class TwitchPage(UILib):
    ...
    def get_twitch_page(self):
        try:
            self.driver.get(twitch_url)
            self.logger.debug(f"Successfully get {twitch_url}")
        except Exception as exp:
            self.logger.debug(f"Exception:{exp}")
            self.logger.debug(f"get {twitch_url} again")
            self.driver.get(twitch_url)
    ...
    def input_text_in_searchbar(self, msg):
        self.send_keys(tlocator.INPUT_search_bar,msg=msg)

    def send_enter(self):
        self.send_keys(tlocator.INPUT_search_bar,msg=Keys.ENTER,need_check=False)
    ...
    def check_streamer_page_is_loaded(self, timeout=10):
        document_ready_state = self.driver.execute_script("return document.readyState")
        self.logger.debug(f"Current url : {self.driver.current_url}")
        if document_ready_state == "complete":
            self.click(tlocator.SPAN_chatroom)
            video_element = self.wait_ele_visible(tlocator.VIDEO_element,timeout=timeout)
            ...
            if ready_state == 4 and not paused:
                ...
                self.logger.debug("The video is not fully loaded or is paused.")
        self.logger.debug("The website is not ready")


    def wait_streamer_page_is_ready(self, timeout = 20):
        n = 0
        while n <= timeout/10:
            n += 1
    ...
    def take_twitch_screenshot(self, filename):
